...
def validate_api_proxy_routes(result: ValidationResult):
    ...
    for route in required_routes:
        ...
        if route_path.exists():
            ...
            if (
                "SPARK_MASTER_INTERNAL_URL" in content
                or "SPARK_APP_INTERNAL_URL" in content
                or "spark-master" in content
            ):
                result.add_success(
                    f"API route {route} uses internal Docker network (CORS safe)"
                )
            elif "localhost:9080" in content or "localhost:4040" in content:
                result.add_error(
                    f"API route {route} uses localhost URLs - will cause CORS issues in Docker!"
                )
            else:
                result.add_warning(
                    f"API route {route} - could not verify CORS configuration"
                )
        else:
    ...
    for interface, props in correct_props.items():
        for prop in props:
            ...
            if re.search(pattern, content):
                ...
                result.add_warning(
                    f"{interface}.{prop} not found - may need verification"
                )
...
def validate_component_property_usage(result: ValidationResult):
    ...
    for tsx_file in components_dir.glob("*.tsx"):
        ...
        for prop_name, pattern in incorrect_patterns.items():
            if re.search(pattern, content):
                result.add_error(
                    f"{filename}: Uses {prop_name} (camelCase) - should be lowercase!"
                )
...
def validate_next_config(result: ValidationResult):
    ...
    if re.search(r"output\s*:\s*['\"]standalone['\"]", content):
        ...
        result.add_error(
            "Missing output: 'standalone' in next.config.js - Docker build will fail!"
        )
...
def validate_docker_compose(result: ValidationResult):
    ...
    for url_var in internal_urls:
        if url_var in content:
            ...
            result.add_warning(
                f"Missing {url_var} in Docker Compose - API proxy may not work"
            )
    ...
    if "spark-network" in content or "networks:" in content:
        result.add_success(
            "Docker Compose defines custom network for inter-service communication"
        )
    else:
        result.add_warning(
            "No custom Docker network found - services may have connectivity issues"
        )
...
def check_direct_api_calls(result: ValidationResult):
    ...
    dangerous_api_patterns = [
        (
            r"axios\.get\(['\"]http://localhost:9080",
            "Direct axios call to Spark Master",
        ),
        (
            r"axios\.get\(['\"]http://localhost:4040",
            "Direct axios call to Spark App UI",
        ),
    ...
    for dir_path in dirs_to_check:
        ...
        for ts_file in dir_path.glob("*.ts*"):
            ...
            for pattern, description in dangerous_api_patterns:
                if re.search(pattern, content):
                    result.add_warning(
                        f"{filename}: Found {description} - may cause CORS issues"
                    )
                    found_issues = True
...
def validate_workers_array_type(result: ValidationResult):
    ...
    if re.search(r"workers\s*:\s*SparkWorker\[\]", content):
        ...
        result.add_error(
            "ClusterStatus.workers typed as 'number' - should be 'SparkWorker[]' (React Error #31)"
        )
    else:
        result.add_warning("Could not verify workers type in ClusterStatus interface")


def validate_tabbed_interface(result: ValidationResult):
    """Check that tabbed interface components exist and are properly configured."""
    print_step("Validating tabbed interface components...")
    ...
    for component in required_components:
        ...
        if component_path.exists():
            ...
            # Verify component exports a default function
            if "export default function" in content:
                result.add_success(
                    f"Tab component {component} exists and exports correctly"
                )
            else:
                result.add_warning(
                    f"Tab component {component} may not export correctly"
                )
        else:
            result.add_error(f"Missing tab component: {component}")

    # Check index.tsx imports the tab components
    ...
    if index_path.exists():
        ...
        for import_name in imports_to_check:
            if (
                ...
                or f"from '../components/{import_name}'" in content
            ):
                result.add_success(f"index.tsx imports {import_name}")
            else:
                result.add_error(f"index.tsx missing import for {import_name}")
    else:
        result.add_error("pages/index.tsx not found")


def validate_logs_api(result: ValidationResult):
    """Check logs API route exists."""
    print_step("Validating logs API route...")

    logs_api_path = FRONTEND_DIR / "pages" / "api" / "logs" / "[container].ts"
    if logs_api_path.exists():
        content = logs_api_path.read_text()
        if "spark-master" in content and "spark-worker" in content:
            result.add_success("Logs API route exists with container support")
        else:
            result.add_warning("Logs API route may not support all containers")
    else:
        result.add_warning("Logs API route not found - logs tab may not work")
...
def print_summary(result: ValidationResult):
    ...
    if result.is_valid():
        ...
        print(
            f"{Colors.GREEN}✅ ALL CRITICAL CHECKS PASSED - Safe to launch!{Colors.NC}"
        )
        ...
        print(
            f"{Colors.RED}❌ VALIDATION FAILED - Fix errors before launching!{Colors.NC}"
        )
...
def main():
    ...
    validate_tabbed_interface(result)
    print()

    validate_logs_api(result)
    print()
