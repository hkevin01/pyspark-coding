...
from pyspark.sql.functions import col, count, sum as _sum, avg, broadcast
...
class IntegrationTestBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.spark = SparkSession.builder \
                .appName(f"IntegrationTest_{cls.__name__}") \
                .master("local[4]") \
    ...
    def tearDown(self):
        ...
        self.logger.info(f"Test duration: {duration:.3f}s")
...
class TestCompleteETLPipeline(IntegrationTestBase):
    ...
    def test_end_to_end_sales_pipeline(self):
        ...
        try:
            ...
            customer_summary = enriched_sales.groupBy('customer_id', 'customer_name', 'region') \
                .agg(
            ...
            # Region summary
            region_summary = enriched_sales.groupBy('region') \
                .agg(
                    count('*').alias('transaction_count'),
                    _sum('amount').alias('total_amount'),
                    avg('amount').alias('avg_amount')
        ...
        try:
            ...
            # Verify counts
            self.assertEqual(customer_result.count(), 100, "Should have 100 customer summaries")
            self.assertEqual(region_result.count(), 5, "Should have 5 region summaries")
            
            # Verify schema
            expected_customer_columns = {'customer_id', 'customer_name', 'region', 
                                        'transaction_count', 'total_amount', 
                                        'avg_amount', 'total_quantity'}
            actual_customer_columns = set(customer_result.columns)
            self.assertEqual(actual_customer_columns, expected_customer_columns,
                           "Customer summary schema mismatch")
            
            # Verify data integrity
            total_transactions = customer_result.agg(_sum('transaction_count')).first()[0]
            self.assertEqual(total_transactions, 1000, "Total transactions should be 1000")
    ...
    def test_multi_stage_transformation_pipeline(self):
        ...
        try:
            summary = enriched_df.groupBy('category', 'value_category') \
                .agg(
                    count('*').alias('count'),
        ...
        self.logger.info("✅ Multi-stage transformation pipeline completed successfully")
...
class TestPerformanceIntegration(IntegrationTestBase):
    ...
    def test_large_join_performance(self):
        ...
        self.logger.info(f"Creating datasets: large={large_size:,}, small={small_size:,}")
        
        # Large fact table
        large_df = self.spark.range(large_size) \
            .withColumnRenamed('id', 'fact_id') \
            .withColumn('dimension_id', (col('fact_id') % small_size).cast('int')) \
            .withColumn('value', col('fact_id') * 1.5)
        
        # Small dimension table
        small_df = self.spark.range(small_size) \
        ...
        self.assertLess(broadcast_time, regular_time * 1.1,  # Allow 10% margin
    ...
    def test_partition_optimization_impact(self):
        ...
        self.logger.info(f"   Improvement: {(default_time - optimized_time) / default_time * 100:.1f}%")
    ...
    def test_graceful_error_handling(self):
        ...
        try:
            ...
            clean_df = df.withColumn(
                'value_clean',
                when(col('value').cast('int').isNotNull(), col('value').cast('int'))
                .otherwise(0)
            )
    ...
    def test_recovery_from_corrupt_data(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"   ✅ Correctly caught corrupt data: {type(e).__name__}")
...
def suite():
    ...
    suite.addTests(unittest.TestLoader().loadTestsFromTestCase(TestPerformanceIntegration))
