...
class BaseTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.spark = (
                SparkSession.builder.appName(f"UnitTest_{cls.__name__}")
                .master("local[2]")
    ...
    def setUp(self):
        ...
        if hasattr(self.__class__, "logger"):
            self.__class__.logger.info(f"Created temp directory: {self.temp_dir}")
    ...
    def tearDown(self):
        ...
        try:
            ...
            if hasattr(self.__class__, "logger"):
                self.__class__.logger.warning(f"Failed to clean up temp directory: {e}")
...
class TestClusterSetup(BaseTestCase):
    ...
    def test_spark_version(self):
        ...
        self.assertGreaterEqual(
            major_version, 3, f"Spark version should be 3.x or higher, got {version}"
        )
    ...
    def test_error_handling_invalid_master(self):
        ...
        with self.assertRaises(Exception) as context:
            ...
            invalid_spark = (
                SparkSession.builder.appName("InvalidMasterTest")
        ...
        self.logger.info(
            f"✅ Correctly caught error: {type(context.exception).__name__}"
        )
...
class TestDataPartitioning(BaseTestCase):
    ...
    def test_partition_size_optimization(self):
        ...
        self.assertTrue(
            all(size > 0 for size in partition_sizes), "All partitions should have data"
        )
        ...
        self.logger.info(
            f"✅ Partition sizes balanced: avg={avg_size:.0f}, max={max_size}"
        )
...
class TestDistributedJoins(BaseTestCase):
    ...
    def test_simple_join(self):
        ...
        df2 = self.spark.createDataFrame(
            [(1, "Engineering"), (2, "Marketing")], ["id", "dept"]
        )
    ...
    def test_broadcast_join(self):
        ...
        result = large_df.join(
            broadcast(small_df), large_df.large_id == small_df.small_id
        )
        ...
        self.assertIn("BroadcastHashJoin", plan, "Should use BroadcastHashJoin")
...
class TestAggregationsAtScale(BaseTestCase):
    ...
    def test_window_functions(self):
        ...
        result = df.withColumn("row_num", row_number().over(window)).withColumn(
    ...
    def test_performance_large_aggregation(self):
        ...
        df = self.spark.range(100000).selectExpr(
            "id", "id % 10 as category", "id * 2 as value"
        )
        ...
        self.assertLess(duration, 10.0, f"Aggregation took too long: {duration:.3f}s")
...
class TestShuffleOptimization(BaseTestCase):
    ...
    def test_coalesce_vs_repartition(self):
        ...
        self.logger.info(
            f"Coalesce: {coalesce_time:.3f}s, Repartition: {repartition_time:.3f}s"
        )
